...
- `Summary` shows Skill, Source, stable Quality, Plugin Eval, estimated tokens, and 30d Usage, ordered by 30-day usage; `Review` adds usage-adjusted Utility and Codex review; `Inventory` emphasizes location and source; `Usage` shows recent/all-time reads and recency; State remains an optional column
...
- Quality is the stable structural score: operational integrity and portfolio clarity are normalized to 100. Utility preserves the usage-adjusted 60-point structure plus 40-point observed-adoption calculation for retention decisions without conflating usage with intrinsic quality.
