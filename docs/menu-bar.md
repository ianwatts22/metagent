...
- four destinations: `Overview`, `Skills`, `MCPs`, and `Projects`, all scoped by one top-level directory picker; the default is all directories
...
- `Summary` shows Skill, Location, Source, Quality, Plugin Eval, Utility, estimated tokens, and 30d Usage, ordered by 30-day usage; `Review` adds Codex review; `Inventory` adds description, upstream owner/repository, version/ref, numeric Weeks old, and reference/script counts; `Usage` shows recent/all-time reads and recency; State remains an optional column
- every installed skill can open a native Get Info sheet from the context menu or Command-I; it shows provenance, lifecycle metadata, content counts, scores, and the canonical path. Editable canonical skills can add or change a portable PNG icon from the context menu or Get Info.
...
- standardized icon-only source cells with accessible tooltips: installed Codex and Claude artwork for their sources, the dotagents mark for genuine dotagents-managed sources, the Vercel mark for Skills CLI, and restrained system glyphs for Git, local, and historical records; self-referential dotagents orphan-adoption records are classified as local rather than as lifecycle ownership
...
- native Command-click multi-selection in the Skills table, with right-click actions to inspect metadata, open skill folders, choose a compatible application, copy canonical paths, copy a concise improvement prompt, or approve manager-aware removal; approved removals disappear optimistically while the authoritative rescan runs, and failed removals are restored by that rescan; plugin selections delegate removal of the owning plugin to Codex's plugin manager, and duplicate plugin targets collapse into one action
...
- Skill provenance is resolved from skills-CLI locks first, then genuine dotagents source declarations, tracked Git repository evidence, and finally explicit user/project-local placement. A dotagents source that points back to its own `.agents/skills/<name>` install directory is orphan-adoption bookkeeping and does not establish lifecycle ownership. A containing Git repository only establishes ownership when Git actually tracks files inside that skill. Agent-specific standalone and plugin sources retain their named manager instead of falling back to `origin unknown`.
