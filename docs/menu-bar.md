...
- every installed skill can open a native Get Info sheet from the context menu or Command-I; it shows provenance, lifecycle metadata, content counts, scores, and the canonical path. Editable canonical skills can add or change a portable PNG icon from the context menu or Get Info using emoji, a curated Lucide picker, or an imported PNG/JPEG/HEIC image. Icon Composer exports can be imported through Image; Icon Composer itself is an authoring app, not a symbol catalog.
...
- the Projects table rolls inventory up by directory: logical skill count, MCP count, actual Claude `.claude/skills` sharing state, genuinely Codex-only skill count, and grouped Doctor issues. `Connected` means `.claude/skills` resolves to `.agents/skills`; `Independent` is allowed only for a separate global collection. Codex-only excludes shared `.agents/skills`, which Codex discovers automatically.
