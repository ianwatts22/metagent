...
- one native inset `Skills` table with persisted `Summary`, `Review`, `Duplicates`, `Inventory`, and `Usage` presets; presets change default columns and sorting while preserving one shared selection, context menu, search, and filter surface
...
- `Duplicates` detects distinct canonical bundles with the same skill identity, while ignoring Claude/Codex projections of one canonical folder. It distinguishes plugin replacements, exact copies, global-plus-project copies that may be intentional for collaborators, and unresolved same-name collisions. Only a similar global standalone copy gets a removal recommendation when a plugin supplies the replacement.
- every installed skill can open a readable skill viewer that separates its name, description, metadata, and rendered Markdown instructions. Get Info remains available from the context menu or Command-I for provenance, lifecycle metadata, content counts, scores, and the canonical path. Context actions can also open the exact `SKILL.md` in its default app or open the skill folder in a detected code editor. Editable canonical skills can add or change a portable PNG icon from the context menu or Get Info using the searchable native macOS Emoji & Symbols palette, all 1,748 icons from pinned `lucide-static` 1.25.0, or an imported PNG/JPEG/HEIC image. Icon Composer exports can be imported through Image; Icon Composer itself is an authoring app, not a symbol catalog.
...
- Plugin usage matches the stable plugin-and-skill identity across marketplace aliases and versioned cache folders. Cache movement from `openai-curated` to `openai-curated-remote` does not reset the displayed history.
