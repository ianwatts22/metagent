...
- `Summary` shows Skill, Location, Source, Quality, Plugin Eval, Utility, estimated tokens, and 30d Usage, ordered by 30-day usage; `Review` adds Codex review; `Inventory` emphasizes location and source; `Usage` shows recent/all-time reads and recency; State remains an optional column
...
- Quality excludes usage and combines available stable signals: Plugin Eval at 60%, structural integrity and portfolio clarity at 20%, and optional Codex review at 20%. Missing optional evaluations are excluded and the available weights are normalized. Utility combines Quality at 70% with observed adoption at 30% for retention decisions.
