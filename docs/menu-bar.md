...
- `Summary` shows Skill, Location, Source, Quality, Plugin Eval, Utility, estimated tokens, and 30d Usage, ordered by 30-day usage; `Review` adds Codex review; `Inventory` adds description, upstream owner/repository, version/ref, relative update time, and reference/script counts; `Usage` shows recent/all-time reads and recency; State remains an optional column
- every installed skill can open a native Get Info sheet from the context menu or Command-I; it shows provenance, lifecycle metadata, content counts, scores, and the canonical path
