...
- the Projects table rolls inventory up by directory: logical skill count, MCP count, actual Claude projection state, genuinely Codex-only and Claude-only skill counts, and grouped Doctor issues. `Connected` means project-owned skills are visible through child links (or a preserved legacy whole-folder link); `Skills CLI` means every shared name is manager-owned; `Independent` is allowed only for a separate global collection. Agent-only counts exclude shared `.agents/skills`; Codex-only also excludes built-in system skills.
...
- Archive… in the same context menu sets unmanaged file-backed skills aside instead of removing them: the bundle and its projection links move to `~/Library/Application Support/Metagent/Archived Skills/NAME/` with an `ARCHIVE.json` manifest, so no agent runtime sees the skill until it is restored. An "Archived · N" toolbar menu appears only while the archive is non-empty and restores each skill to its exact recorded paths (or reveals the archive in Finder). Managed skills and plugins are not archivable because a pure file move would desync their manager state; skill history records these transitions as `archived`/`restored` rather than `removed`/`added`
- one activity badge and one reload button in the top control line are the only places processing is reported or restarted. The badge covers skill scanning, session-history indexing, MCP configuration checks, and Plugin Eval in one shape, showing determinate progress where a total is known; attention states outrank work in progress, because a stalled index stays true while an unrelated scan runs. The badge is indicator-only — a circular control the same size as the reload button, holding the spinner, gauge, or warning glyph — so a long report like "Plugin Eval 13 of 107" cannot widen the control line and crowd the tab strip; the full wording lives in the badge's tooltip and accessibility label. While work runs the badge takes over the reload button's slot, since reload would be a no-op then anyway. It stays hidden when the app is idle. Reload rescans skills and Doctor findings, rechecks MCP configuration, and continues indexing history. Destination pages carry no refresh control and no status line of their own; the Doctor findings sheet keeps its own rerun because it is a modal.
...
- Three tables carry the record. `history_snapshots` and `history_metrics` hold one narrow row per metric per scope per day, so adding a metric never needs a migration. `history_skill_states` is a slowly-changing dimension that writes a new row only when a skill's shape changes, which keeps it small and makes it the source for change events. `history_events` holds added, removed, renamed, updated, content-changed, source-changed, and scope-changed transitions. Plugin-cache skills are keyed by their marketplace identity rather than their versioned cache path, so a plugin update is one `updated` event carrying the version transition instead of a removal plus an unrelated addition; a one-time migration rewrote earlier path-keyed rows to the same identities so timelines stayed continuous across the change.
...
- A `Trend` control in the Skills header sets how far back every sparkline and delta looks: 7d, 30d, 90d, or All. It governs the charts only and never changes what a card counts, because only one of the six metrics has a window of its own. "Unused, last 30 days" measures a trailing 30-day window on each day it is sampled; "Never used" and the reads distribution are all-time; tokens and age are point-in-time. A per-metric range would be incoherent for five of the six, and "never used, last 7 days" is a contradiction.
- The chart's own window is stated where the chart is: the word `Trend` sits beside the control, and each sparkline carries its range in the corner. Without that, a line under a card titled "Unused, last 30 days" reads as though the title set its range, and the two meanings of thirty days silently merge.
- The control appears whenever any history exists rather than only when the chosen range has something to plot. Gating it on the latter would hide it the moment a narrow range came back empty, leaving no way to widen it again.
- Each Overview metric card gains a delta against the oldest day in the window and a sparkline beneath it. The delta names the day it compared against rather than claiming an exact window it may not have, because a machine that was closed for a week has no 30-day-ago sample. A single recorded day shows no delta at all: one value is a value, not a trend, and a zero delta there would be comparing a day against itself.
...
- Development is `~/Applications/Metagent Dev.app`: bundle ID `com.ianwatts.metagent.menu-bar.dev`, name `Metagent Dev`, version `0.0.0-dev` with the build commit recorded in `MetagentBuildCommit`, and a badge dot on the menu bar glyph so two otherwise identical icons stay tellable apart. The dev channel also ships its own app icon — a separately authored rough-ink rendition of the glyph compiled from `metagent-icon-dev.icon` through the same actool pipeline, so Dock and app switcher distinguish the channels at a glance with the full set of macOS 26 appearance variants. Because unversioned bundles claim the checked-in 0.1.0/1, every build without an explicit `METAGENT_VERSION` sheds its Sparkle keys, which also defuses the prod-branded bundle `verify.sh` leaves in `dist/`. About identifies dev builds by commit rather than by the meaningless version, and `install-app.sh --restart` verifies the old process actually exited before relaunching, escalating past a silently failed polite quit. Every Sparkle key is deleted from dev builds; the startup policy treats a missing feed as unconfigured and never starts the updater. This is what makes the split load-bearing rather than cosmetic: a local build is stamped below every public release, so a dev build carrying the live feed would eventually be "updated" back to an older shipped version by its own updater.
