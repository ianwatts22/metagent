...
- every installed skill can open a native Get Info sheet from the context menu or Command-I; it shows provenance, lifecycle metadata, content counts, scores, and the canonical path. Editable canonical skills can add or change a portable PNG icon from the context menu or Get Info using the searchable native macOS Emoji & Symbols palette, all 1,748 icons from pinned `lucide-static` 1.25.0, or an imported PNG/JPEG/HEIC image. Icon Composer exports can be imported through Image; Icon Composer itself is an authoring app, not a symbol catalog.
...
- one persisted Group by picker applies to Summary, Review, Inventory, and Usage. Source, Location, and Upstream create native collapsible table groups without duplicating the table into separate views.
